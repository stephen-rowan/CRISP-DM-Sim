...
1. **Given** a user has submitted multiple experiments, **When** they view the dashboard, **Then** they see a list or timeline of all their experiments with key identifiers (name, dates, circle)
...
- What happens when a user tries to submit duplicate experiment names? (System should either allow duplicates with unique identifiers or prevent duplicates with clear messaging)
...
- **FR-002**: System MUST collect experiment lead name, circle, start date, and end date (if applicable) in the Experiment Details section
...
- **FR-008**: System MUST persist submitted experiment data so it can be retrieved for dashboard display
...
- **Experiment**: Represents a single CRISO experiment submission. Key attributes include: experiment lead name, circle, start date, end date (optional), problem statement, baseline statement, purpose/objectives, design information (who was involved), group size (number of people), method description, success criteria, analysis approach, evaluation plan. Each experiment has a unique identifier and timestamp of submission.
...
- Experiment data will be stored locally (file-based storage) for the initial implementation
